...
**Applies to:** diluvium, diluvium-drt, dollup, aloelite, xtrshow,
aloeschema, and any Aloecraft project that publishes a release.
...
- [ ] **Record the schema.org release as a fact.**
      `src/data/schemaorg_current.py` is a vendored snapshot of the schema.org
      ontology and nothing names which release it is — the top level holds
      only `@context` and `@graph`, and none of its 3,187 graph nodes carries
      a version marker. Record it as `schemaorg_release`, in the changelog
      entry and in `BUILDINFO.txt`. This is §1's rule and not a nicety:
      `load_schema_org(fetch=True)` fetches `version/latest` at runtime, so
      the same call can return a different ontology than the vendored one,
      and today no consumer can tell which one they got.
- [ ] **`publish.yml` publishes any `v*` tag, ungated and unfiltered.** It
      depends on no test job — `main.yml` runs tests on the same tag push,
      in a separate workflow, so the upload races them and proceeds either
      way. And PEP 440 normalises `v0.4.0-dev.7` to `0.4.0.dev7`, a valid
      PyPI version, so adopting §7 as written would push every nightly to
      PyPI. Exclude `-dev.` tags, and decide the test gate.
- [ ] **`version.mk`'s `_sync_version` stamps a Cargo path.** It writes
      `.package.version`, but `pyproject.toml` has no `[package]` table —
      only `[project]` — so the target has never stamped anything. The
      shared copy does this through `stamps` instead: a pattern on
      `pyproject.toml` holding the PEP 440 spelling.
- [ ] `.technoproj`: `build: 0` → `pre: null`
- [ ] Gains `CHANGELOG.yaml` and the engine, with `tag_rule: "exact"`
- [ ] **Decide whether aloeschema is mirrored** — the same open question as
      xtrshow. It has no release workflow and no GitHub release has ever
      been cut. PyPI-only, and it should say so in `mirrors.json`; mirrored,
      and it needs artifacts, `BUILDINFO.txt` and `SHA256SUMS.txt`. Note
      that §4 does not reach the wheel or the sdist — those names are fixed
      by PEP 427 and PEP 625 and pip depends on them. §4 governs the release
      assets published beside them.

Already conforms: tag spelling — `v0.2.3` is already `tag_rule: "exact"` —
and `.technoproj`, `pyproject.toml` and the newest tag all agree at `0.2.3`.
Nothing enforces that agreement, but nothing has drifted either.
